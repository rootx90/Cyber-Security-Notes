...
- in BurpSuite , these tools of are lesser known tools

### 1. Decoder & Comparer
- less imp tools but they are essential to know as a proficient webapp tester
- Decoder
	- allows us to perform various transforms on pieces of data - means we can do both encoding & decoding ✔️
...
- Ques
	1) Let's first take a look at decoder by revisiting an old friend. Previously we discovered the scoreboard <br>
		...
		- STEP 2: in burpSuite -> Target -> Site Map , <br>
			in `http://10.10.191.203` IP address folder -> api -> challenges -> `score%20Board` file -> right click on it <br>
			& click `send to repeater`
		- STEP 3: in repeater tab , right click on "request" section -> click `send to Decoder`
	2) Copy the first line of that request & paste it into decoder. Next, select 'Decode as ...' URL
		- so we need to "Decode as URL"
		- STEP 4: in Decoder tab , on right , select `Decode as URL` like this 
			<br><img src="../../notes-pics/02-Module/11_lecture/11_lecture-0-M2.jpg" alt="" width="500"/>
        - output : at the bottom , previously scoreboard was `Score%20Board` , now that `%20` become a empty space
	...
	4) Similar to cyberChef, Decoder also has a 'Magic' Mode where it will automatically attempt <br>
		...
		- here click on `Smart Mode` btn , so automatically we'll get the same result as a output of STEP 4
			<br><img src="../../notes-pics/02-Module/11_lecture/11_lecture-0-M2.jpg" alt="" width="500"/>
	...
	6) Comparer can perform a diff against two different metrics, which one allows us to examine the data loaded <br>
		...
		- STEP 1: in burpSuite -> "repeater" tab -> let's say click on `send` btn , output : a Response we got
		- STEP 2: right click on "Response" section (which we got) -> click `send to comparer` & <br>
			then in "Request" section , change the Host as `google.com` -> again click on `send` btn <br>
			& again right click on "Response" section -> click `send to comparer`
		- STEP 3: in comparer tab, output : u'll get those 2 different response in compare wise 
			<br><img src="../../notes-pics/02-Module/11_lecture/11_lecture-1-M2.jpg" alt="" width="500"/>
		- STEP 4: at the right bottom , click on `words` btn , output : u'll see only time different b/w those 2 responses like this 
			<br><img src="../../notes-pics/02-Module/11_lecture/11_lecture-2-M2.jpg" alt="" width="500"/>
    		<br>- below we can see Key: as modified color, so comparer automatically tells everything
			<br>- so "modified" label color will be same as highlighted , so only difference b/w those 3 responses i.e time
			<br>- so this is the usecase of comparer

### 2. Extender
- means to increase the functionality , we have mods 
    <br>- means in burpSuite, we have specific extension that we can install (which as for community version)
...
- Other modules
	...
	- STEP 2: examine that file & Questions are : 
		- Q 1 : Download the report attached to this task. What is the only critical issue ?
			- Ans : Cross-Origin resource sharing: arbitrary origin trusted
