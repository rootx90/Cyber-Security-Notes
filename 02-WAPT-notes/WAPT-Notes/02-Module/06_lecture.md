...
- BurpSuite has a SSL Certificate 
    - like open it -> go to it's `proxy` tab -> in `intercept` tab , output : u'll see `Open Browser` button
...
- intercepting via BurpSuite with `Open Browser` burp suite feature
	- go `Proxy` tab -> intercept tab -> click on `Open Browser` button ✔️
	- now when u type like `demo.testfire.net` website , then inside BurpSuite <br>
		that website gets intercept like this ✔️ <br><img src="../../notes-pics/02-Module/06_lecture/06_lecture-0-M2.jpg" alt="" width="500"/>
	...
	- & which website is captured/intercept will be shown in `HTTP history` tab ✔️
	- the browser on which proxy is not configured then if searching stuff happen on that <br>
		then we can't intercept the via burpsuite
- intercepting via BurpSuite without using `Open Browser` burp suite feature
	- so we can't to intercept what we're running browser manually <br>
	...
	- STEP 2 : click on `settings` button -> select only `manual proxy config` & others should be unchecked <br>
		...
		(which is a localhost) & port as `8080` like this <br><img src="../../notes-pics/02-Module/06_lecture/06_lecture-0-M2.jpg" alt="" width="500"/>
	- STEP 3 : click ok
	- STEP 4 : in BurpSuite -> Proxy tab -> options tab -> localHost must be same as we set/define inside the browser <br>
		<img src="../../notes-pics/02-Module/06_lecture/06_lecture-0-M2.jpg" alt="" width="500"/>
	- STEP 5 : using burpSuite for MITM (man in the middle attack)
		...
		- Diagram : <br>
			<img src="../../notes-pics/02-Module/06_lecture/06_lecture-0-M2.jpg" alt="" width="500"/>
	- when we hit the request for that website then the request will not go directly to the server , to the Burp Suite instead , here `Burp Suite` is a `proxy server`  ✔️ 
	- Why this will happen - cuz we gave a port of that localhost ✔️ 
	- so firstly request will go from Client to that Burp Suite proxy server then we'll intercept the request & after intercepting the request , if u as a hacker want to forward to the server then only request will go the server ⭐
		- then the server's response came to the burpSuite Proxy server then client ✔️
		- so this is MITM attack , Role of the proxy server is to intercept each request & responses also ⭐
		- right now , in Proxy tab -> Intercept -> intercept is off , but if u keep it off then still we'll get that URL in `HTTP history` but on the intercept by clicking on `Intercept is on` button
		...
		- STEP 8 : now , response will come 
			...
			- & then double click on that URL , u'll get the response like this <img src="../../notes-pics/02-Module/06_lecture/06_lecture-0-M2.jpg" alt="" width="500"/>
	...
	- intercepting via BurpSuite (setting up SSL Certificate issue with https)
		...
		- we'll get issues with HTTPS when we intercept coz each `HTTPS` website need SSL certificate ✔️
		- Eg : in firefox -> write https://demo.testfire.net then we'll get error <img src="../../notes-pics/02-Module/06_lecture/06_lecture-0-M2.jpg" alt="" width="500"/>
		...
		- installing SSL certificate
			...
			- now u'll get the page of burp suite <br><img src="../../notes-pics/02-Module/06_lecture/06_lecture-0-M2.jpg" alt="" width="500"/>
		...
		- role/purpose of SSL certificate in HTTPS ⭐
			- each HTTPs website needs a SSL certificate , so that website/webapp send the request/data to the web server , that data/request will be in encrypted ✔️
			- Eg : <img src="../../notes-pics/02-Module/06_lecture/06_lecture-0-M2.jpg" alt="" width="500"/>
				- u'll get verified by : PortSwigger cuz on firefox , we installed the SSL certificate of PortSwigger ✔️
				- means each HTTPS website will get intercept by BurpSuite & no error will come cuz SSL certificate installed on this browser ✔️
	...
	- we have `foxyProxy` firefox extension - which u can use if u don't wanna do that like going on settings -> preference -> & then searching `proxy`  & setup the localhost for the burp Suite proxy browser like this <img src="../../notes-pics/02-Module/06_lecture/06_lecture-0-M2.jpg" alt="" width="500"/>
