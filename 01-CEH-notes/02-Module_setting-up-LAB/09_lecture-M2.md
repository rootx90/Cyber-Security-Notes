...
1. `pwd` : to show the current path that u're in right now
	...
	- Eg : <br>![](09_lecture-0-M2.jpg)
2. `ls` : means list
	- used to show the files & folders of the current path
	- Eg : <br> ![](09_lecture-1-M2.jpg)
...
- `cd` commands
	...
	3. `ls -l` : in GUI , we can check size of folders & files via right click on a folder/file -> click properties
		...
		- Eg : <br>![](09_lecture-4-M2.jpg)
		- Ques : the root & root , kali , kali - means Who made this ? <br>Ans : means Eg : AhMyth folder made by root & root
...
- hidden files & folders commands
	...
	1. `ls -a` : to see hidden files 
		...
		- Eg : <br>![](09_lecture-5-M2.jpg)
- make a folder 
	- in GUI , u'll go to that folder & u'll create a folder
	- `mkdir` : means make directory/folder
	- Eg : <br>![](09_lecture-7-M2.jpg)
- delete a folder
	- `rmdir` : remove a directory 
	- Eg : <br>![](09_lecture-8-M2.jpg)
	- Eg : empty folder can't be deleted via `rmdir` or `rmdir -r` <br>cuz here , rohan folder contains inside itself a folder
		- 1) ![](09_lecture-9-M2.jpg)
		- 2) ![](09_lecture-10-M2.jpg)
	...
	- `rm` : delete a folder which is not empty ✔
		- Eg : `rm -r rohan` <br>![](09_lecture-11-M2.jpg)
- copy & paste a folder inside the another folder
	- `cp` : to copy & paste a folder inside a other folder
		- Eg : `cp aditya /home/kali/Documents` <br>![](09_lecture-12-M2.jpg)
		- Advice : whenever error comes then we'll the solution also , so read the error carefully ✔
		- Eg : `cp -r aditya /home/kali/Documents` , so here `-r` doing forcefully <br>![](09_lecture-13-M2.jpg)
		- Ques : why system makes us to use `-r` <br>Ans : cuz when system think that folder is imp then the system says to use `-r` to do forcefully ✔
- move/cut & paste a folder from one place to the another place
	- `mv` : command to move a folder i.e "aditya" to desktop
	- 1) Eg : `mv aditya desktop` <br>![](09_lecture-14-M2.jpg)
	...
	- 2) Eg : moving a folder to another folder (which is far away from that folder which we want to move) <br>![](09_lecture-15-M2.jpg)
- seeing files & folders of a different path from sitting a different location without changing location
	- means we want to see inside Desktop folder - without changing current location <br>that we're in let's say Documents ✔
	- 1) Eg : `ls /home/kali/Desktop/desktop` <br>![](09_lecture-16-M2.jpg)
	- 2) Eg : `ls /home/kali/Desktop/` <br>![](09_lecture-17-M2.jpg)
- how to create or see a txt file 
	- in windows OS , we use tool like notepad , wordpad , etc
		...
		- 2) Eg of Windows OS : `notepad vss.txt` - run the command again <br>& now this file will get open cuz we have the file <br>![](09_lecture-18-M2.jpg)
...
- `cat` command : how to see stuff written inside the txt or any file 
	- Eg : `cat arvind` & no need to define file extension in Kali Linux ✔ <br>![](09_lecture-19-M2.jpg)
...
- How to come at "home" folder of kali , doesn't matter how deep we're in which folder
	- just write `cd` & Eg : <br>![](09_lecture-20-M2.jpg)
...
- `chmod` command : to give & remove single or multiple permissions to a folder/file
	...
	- 1) Eg - to give/add the permission from the file/folder : <br>we want to give execute permission to "arvind" file -> `chmod +x arvind` <br>so `+`  means adding a mod to that folder/file & name of mode i.e `x` , so `+x` add executable mode <br>to "arvind" folder ✔ <br>Output : <br>![](09_lecture-22-M2.jpg)
	...
	- 3) Eg : to remove multiple permissions from the file/folder <br>-> `chmod -xwr arvind` OR  `chmod -777 arvind` <br>Output : all permissions removed from "arvind" folder/file<br>![](09_lecture-23-M2.jpg)
	- 4) Eg : to give read , write & execute permissions back to "arvind" folder/file <br>-> `chmod +xwr arvind` OR `chmod 777 arvind` OR `chmod +777 arvind`<br>Output : all permissions added in "arvind" folder/file <br>![](09_lecture-24-M2.jpg)
- Ways to execute a file in terminal : Now let's execute a file i.e "arvind" - after giving it -> a execute permission
	...
	- Eg of `./` : run the "arvind" file only `./arvind` <br>![](09_lecture-25-M2.jpg)
	...
	- Eg of `./` : now let's remove the executable permission from "arvind" file -> `chmod -x arvind` <br>Output : now execute the file -> `./arvind` - then we'll get error ![](09_lecture-26-M2.jpg)
	- & same with `bash` command - For Eg : <br>![](09_lecture-27-M2.jpg)
- 4 ways to install a software 
	- like if u want to install chrome browser then 
		...
		- 3rd way : github
		- 4th way : apt 
	- apt : is like a playstore in linux - For Eg : `apt update` - then u'll get permission denied error <br> ![](09_lecture-28-M2.jpg)
	...
	- Ques : why we need admin/root permission <br>Ans : cuz before this , we were doing small things. Now doing advance stuff , we need admin permission ✔
	- to get the admin/root permission
		- `sudo su` : here `su` means super user ✔ <br>now it'll ask for password of Kali OS : i.e by-default password <br>![](09_lecture-29-M2.jpg)
- using `apt`
	- so `apt` is a playstore to install softwares
	- if u don't know about `apt` then `apt --help` <br>![](09_lecture-30-M2.jpg)
	...
	- Ques : in which folder , setting/configuration exist <br>Ans : i.e "etc" folder
	- to do setting of `apt` - so do `etc/apt`& then do `ls` <br>Output : u'll see sources.list & "sources.list~" => means "sources.list~" file temp file <br>cuz sources.list file not saved <br>![](09_lecture-31-M2.jpg)
	- write `nano` to open nano text editor , cuz "gedit" text editor is not installed by-default ✔ <br>so run this => `nano sources.list` <br>now write this <br>![](09_lecture-32-M2.jpg)
...
### Ques : 
1. Folder vs File
	- File - is a program & folder - is where we put all the programs inside of a folder
2. Ques : why hashtag removed from inside the `sources.list` <br>Ans : cuz `#` used for comment the line & by-default we need to do settings in Kali Linux <br>cuz it won't allow or give permission to download or install <br>- that's why that hashtag removed ✔

- Extra Info : [40+ Basic Linux command - YouTube](https://www.youtube.com/watch?v=ZVT-BKNyaTg&ab_channel=HackerJohn)
- How to Update and Upgrade Kali Linux Operating System? How to install Kali latest Version? : <br>[Update & Upgrade Kali Linux OS? How to install Kali latest Version? - YouTube](https://www.youtube.com/watch?v=VPW61aWws_4&ab_channel=TechnicalHaroon)
