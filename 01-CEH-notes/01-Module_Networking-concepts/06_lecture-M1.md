...
> 1) What is a HTML request ?
> 2) What is a HTML response ?
> 3) Types of HTTP Request methods/ways - to send the data
...
- Pic <br>![](06_lecture-0-M1.jpg)
- Ques : what this actually green box is ? where it is use ? for what purpose it's use
	- Ans : so in 5th lecture , we saw that when "System A" did a packet request i.e google.com <br>- Ques : then where that packet request will go first i.e to DNS <br>then that packet request will be forward to NS1 & NS2 - for zone file - cuz the Domain setting <br>which we saw  - that setting is of the domain name server's page & NS1 & NS2 stay in server <br>so the packet request will go to the server (of that Domain Name) after the DNS ✔
	- so this green box is a packet request 
- understanding a packet request
	- Ques : why system generate or send request packet to the server ? for what purpose it's use ?<br>Ans : so that to load a website on our system , so in response - client got a complete code of a website <br>& meta data , response are big topics which comes in penetration testing , <br>so we'll not go in too deep in this course about response ✔
	...
	- `Request line` : is a main line in a packet request ✔
		- Eg : `GET /doc/test.html HTTP/1.1` : means in that website <br>there is a "doc" is a folder & inside this folder - there's a "test.html" file
		- `HTTP/1.1` : means which protocol/rules-&-regulation that file using to get transfer i.e http <br>& 1.1 - means version of http protocol
	...
	- Ques : what is Header means ? ✔<br>Ans : u can say `request line` is a hair - means firstly URL of the website come <br>then "request header" - here header means head of human body
	- `Accept` header : means Eg : `Accept : image/gif, image/jpeg, */*`
		- here means the request that we sent to the website , we told in the request <br>that our system which type of image format can understand ✔
		- so gif & jpeg are format/extension of images
		- so our system can understand this 2 types of image format ✔ <br>so that when that website's server reply then send the image in that format only
	...
	- `Accept-Encoding` header : 
		- Ques : what is encoding ? <br>Ans : from internet , when u load a zip file (means compressed file) , so these zip called encoding <br>means when these file made in zip then those file stored in zip via encoding process <br>due to which become more secure , that's why called encoding file ✔
	...
	- `User-Agent` header : `v imp ⭐`
		...
		- means in a packet request , this header contains our system's browser name & browser version via <br>which we made the packet request & our OS details like name of OS & version of it , essential info about our system ✔
	...
	- in body section , all those things comes what we're asking from the server 
		- Eg : in facebook , for login - we tell username & password <br>so that username & password will go via in body
		- so inside the packet request , after a space - that's called body
		- Ques : why these info goes in body ✔<br>Ans : cuz if those info are in header then anyone can see those details <br>if u see URL inside the browser , but u never saw a body of a packet request <br>cuz body section is secure ✔
		- so sensitive stuff will go/come inside the body section of a packet request ✔
...
- A HTML response is a packet providing Permission to access website and content <br>Includes GET/POST , Headers & Body (complete program of that page)
- Ques : A HTML response is a packet providing Permission to access website and content ? <br>is this statement correct ? ✔<br>Ans : this statement - means whenever a website reply then no permission asked to access it<br>No cuz , whenever u ask sensitive page/info then the website will not allow & it'll stop u from access it <br>- so , response doesn't mean that in reply , always u'll get the website , sometimes website will <br>not allow u to access those info <br>- so "not allowing to access" also kinda response from the website ✔
...
- HTTP Request Methods 
	- Pic 1 : <br>![](06_lecture-1-M1.jpg)
	- Pic 2 : <br>![](06_lecture-2-M1.jpg)
...
- `HEAD` method : 
	...
	- Ques : what is Status line in header section ✔<br>Ans : it tells where reached & when the data reached & is everything safe or not 
- `POST` method : 
	- it sends all the info & files in body section - so this method is secure & anyone can't see <br>except URL
	- that's why nowadays , mostly POST method used
- `PUT` method :
	- this method is dangerous cuz <br>only admin has access of PUT method 
	...
	- Eg : in that packet request , is a hacker has a access of PUT method <br>then the hacker can replace that file request i.e `test.html` to `malware.com` ✔ <br>that's why access of it has only with admin <br>& if the website is vulnerable then users also can access this method ✔
- `DELETE` method :
...
- `OPTIONS` method :
	...
	- Eg : this method let's say wants to make a communication via TCP protocol. <br>so it'll decide which protocol + when to make the communication via which protocol + etc ✔
	- so this method will give options to client/users for how to connect with the server <br>so whatever this method have ways/methods to connect with the system <br>all those options - this method will give ✔
- `TRACE` method :
	- Eg : in windows PC , if any file u didn't get then u just go <br>Ques : what u do without going in drive or folder <br>Ans : so u try to search the location of the file - due to this u got the complete location & <br>in which drive that file exist ✔
	- it used to give access/location of a thing which are in a website <br>Rule : on internet , until u don't have file or folder or a website location then u can't access it ✔ <br>so same way , this method give us location of thing which are inside of any website
...
- `CONNECT` method : 
	...
	- Conclusion of `CONNECT` method : ✔
		...
		- so `CONNECT` method guide to that protocol for when to establish the connection <br>& when to break the connection
...
- 1) Mine Advice : I saw that in every lecture ,
...
- 2) Advice : if u start directly practical then that will be waste of time 
...
- 3) Advice : understand those topics or stuff which are imp to u & which are gonna help u in ethical hacking
...
- 4) we'll see about onion link when we use tor browser
...
- 5) Advice : try to use kali linux as much as possible , so that u can get the good understanding <br>then u use other linux OS like Parrot OS
- 6) Ques : someone did something via Kali Linux & he/she installed the kali in VMware in main system <br>so can we reach to his/her from VMware to main system ✔
	- Ans : Yes , cuz VMware installed in ur actual System , so if any info goes from VMware to browser 
	- Ques : browser is wide wide or LAN network ? <br>Ans : it's world wide , so here IP address will be Public
	- Ques : so is VMware has that Public IP address ? <br>Ans : No , & even this Public IP address doesn't have in ur system too ,<br>ur system has a private IP address which is given by the router
	- Ques : is IP address of System or router ? which one will go ? <br>Ans : Router , cuz while setting up the router , u gave aadhar card <br>so details of aadhar card is linked with the router 
- 7) we'll see how to make urself secure & safe - it's a part of anonymous