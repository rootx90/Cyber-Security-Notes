...
- STEP 5 : `services`
	...
	- now let's so towards Apache Tomcat i.e port no. 8180 & it's protocol = http <br>![[18_lecture-0-M3.jpg | 500]]
	- "Filtered" Port : means those ports which are not properly neither "close" nor "open" ✔ <br>& these sort of ports are not useful for us 
		...
		- "filtered" ports are half opened & half closed : <br>- Pic of Filtered Port : <br>> [1 Pic of only "filtered" status ports](https://www.hackingarticles.in/nmap-for-pentester-port-status/)<br>> [2 Pic of only "filtered" status ports](https://www.researchgate.net/figure/Figure-Nmap-detects-a-filtered-port_fig1_336838917)
...
- STEP 6 : going towards Apache Tomcat i.e port no. 8180 & it's protocol = http
	...
	- so let's use the 6th i.e <br>![[18_lecture-1-M3.jpg | 500]]
	- Advice : how to find useful exploits related to that service ✔<br>1st : check the software/product running on that service which we got via command i.e "search tomcat linux" <br>2nd : & check inside output of "search tomcat" anything which is similar & related to that service 
	- STEP 6.4 : run `use 6` , output : our exploit will be started <br>![[18_lecture-2-M3.jpg | 500]]
	- now let's see options
	- STEP 6.3 : run `options` : means what things required for that exploit  , output : ![[18_lecture-3-M3.jpg | 500]]
	- this exploit - whatever things it doesn't want marked as no in their respective Name <br>- so it needs RHOSTS , RPORT
	- Ques : what RHOSTS should be ? <br>Ans : i.e address/IP-address of that Victim's system
	- Ques : what's the command to set the RHOSTS or change the previous RHOSTS with a new one <br>Ans : i.e `set rhosts
	- Ques : what does tomcat means <br>Ans : is a Java application server designed to **deploy Java Servlets and JSPs on your system** <br>- it is one of the most widely used Java applications and web servers.
...
- STEP 8 : now check whether RHOSTS & RPORT are added or not 
	- run `options` , output <br>![[18_lecture-4-M3.jpg | 500]]
- STEP 9 : run `exploit` , 
	- output : <br>![[18_lecture-5-M3.jpg | 500]]
	- in output : "exploit completed, but no session was created" , cuz there's is a reason
	- might be the reason is 
		- Pic : <br>![[18_lecture-3-M3.jpg | 500]]
	...
	- STEP 9.1 : getting understanding about more on "Tomcat" for username & password , run `search tomcat`
		...
		- so we'll find those Auxiliaries which are related to username , password or login , Eg : i.e 27th <br>![[18_lecture-6-M3.jpg | 500]]
		- so 27th Auxiliaries is tomcat_mgr_login , which is same as both which we saw i.e <br>![[18_lecture-7-M3.jpg | 500]]
		- so let's use "tomcat_mgr_login" Auxiliary
	- STEP 9.2 : run `use 27` , output : now that Auxiliary is set as Auxiliary <br>![[18_lecture-8-M3.jpg | 500]]
- STEP 10 : now let's check the options are required for that Auxiliary ✔, run `options` 
	- output : ![[18_lecture-9-M3.jpg | 500]]
	- & whatever that auxiliary needs , it already filled by itself <br>so it also need RHOSTS , RPORT - so let's set these 2
	- STEP 10.1 : run `set rport 8180` & then run `set rhosts 192.168.224.128` , output <br>![[18_lecture-10-M3.jpg | 500]]
...
- STEP 11 : finding login & password , so run either `run` OR `exploit` - but we'll do `run`
	- output : <br>Pic 1 : ![[18_lecture-11-M3.jpg | 500]] <br>Pic 2 : ![[18_lecture-12-M3.jpg | 500]]
	- now in output , it's doing kindof brute forcing on Victim's system - means it's trying to bring username & password of HTTP ✔
	- in output 100% completed , now find in output - is there any "correct" written or not i.e <br>![[18_lecture-13-M3.jpg | 500]]
...
- STEP 12 : run `use 6` & then run `options`
	- output : ![[18_lecture-14-M3.jpg | 500]]
	- now everything is added <br>now we need to give username & password of http
	- STEP 12.1 : run `set HttpPassword tomcat` - to add the http password <br> & run `set HttpUsername tomcat` - to add the http username <br> ![[18_lecture-15-M3.jpg | 500]]
	- STEP 12.2 : let's exploit the "tomcat_mgr_deploy" , run command `run` , output : <br>![[18_lecture-16-M3.jpg | 500]]
	- STEP 12.3 : let's use "exploit" command , so run `exploit` , output : <br>![[18_lecture-17-M3.jpg | 500]]
	...
	- STEP 12.4 : run `options` , output : checking whether why exploit didn't worked properly <br>![[18_lecture-18-M3.jpg | 500]]
	- in output , it set the our system's LHOST & LPOST by-default , so let's try to `run -j` for exploit
	- STEP 12.5 : run `run -j` , output : still we'll get error output as in STEP 12.3 i.e 500 internal Server Error <br>- & when we do `ls` then we'll get our system's files & folders <br>![[18_lecture-19-M3.jpg | 500]]
...
- STEP 2 : run `armitage` 
	...
	- STEP 2.1 : a pop will come & click on "connect" button & click on "Yes" button <br>- output : all the services will be started - just like we did to use `msfconsole` tool <br>- output : once all the services completed - then GUI of Armitage tool will get open <br>![[18_lecture-22-M3.jpg | 500]]
...
- STEP 4 : to check services for a particular software/product
	- STEP 4.1 : so search on this bar i.e `vsftpd`<br>![[18_lecture-23-M3.jpg | 500]]
...
- STEP 5 : how to use that exploit of that software
	- STEP 5.1 : just drag the vsftpd of exploit & drop on that machine <br>![[18_lecture-24-M3.jpg | 500]]
	- output : we'll get a popup <br>![[18_lecture-25-M3.jpg | 500]]
	...
	- STEP 5.2 : click on "launch" button , output : now just focus on that machine icon - whether machine changed or not <br>![[18_lecture-26-M3.jpg | 500]]
	...
	- STEP 5.3 : click on that machine icon -> click on "shell 1" -> we have options like interact <br>- so click on "interact" , output : we'll get a terminal <br>![[18_lecture-27-M3.jpg | 500]]
...
- STEP 6 : let's see a different service/software like tomcat
	...
	- STEP 6.3 : now that pop will come & it check by-default i.e "use a reverse connection" <br>![[18_lecture-28-M3.jpg | 500]]
	...
	- STEP 6.4 : click on "launch" button , output : "commend shell" comes - then means Victim's system hacked again <br>![[18_lecture-29-M3.jpg | 500]]
	- STEP 6.5 : right click on Victim's system icon & we'll get "Shell 2" - cuz we hacked the system 2nd times <br>STEP 6.5.1 : click on "interact" , output : run `ls` - then we'll get a different file cuz due to the service/protocol <br>![[18_lecture-30-M3.jpg | 500]]
...
1. let's say u don't have social Media account of that person then how u can do IG about that person
...
2. to hack a hacker then it's bit difficult cuz
	- whatever the things u know - he/she also knows that's why
	- & what if he/she knows more than u  
3. Q : when we do search then many exploits comes - then how to know useage of each exploits
