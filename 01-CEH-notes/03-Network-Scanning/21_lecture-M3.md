...
> 1) understanding : What is URL
> 2) understanding tool : System Hacking Using Camphish (URL tool)
...
- Q : do u think a system can be hacked via URL <br>Ans : a URL is a address of those things - which we access on the internet
	...
	- Examples of URL : 
		- Eg 1 : wscubetech.com" = will show the main page of wscubetech ✔<br>- but let's say we want to do ethical hacking course online , so URL will be "`https://www.wscubetech.com/online-ethical-hacking-course.html`" <br>- so behind the scene , here "online-ethical-hacking-course.html" folder is opened ✔
...
### understanding tool : System Hacking Using Camphish
...
- About Camphish tool for URL hacking ✔
	...
	- Conclusion Pic : <br>![[21_lecture-0-M3.jpg | 500]]
...
- STEP 4 : 2 ways to execute this file , but we'll use "./" , run `./camphish.sh`
	- output : <br>![[21_lecture-1-M3.jpg | 500]]
	- in output , we're getting 2 options - means which one server do u want to use to take the localhost to world wide ✔
- STEP 5 : press `1`
	- output <br>![[21_lecture-2-M3.jpg | 500]]
	- in output : we're getting Template <br>Q : which one have good camera <br>Ans : Online meeting 
	- STEP 5.1 : press `3` , output : <br>![[21_lecture-3-M3.jpg | 500]]<br>- in STEP 5.1 output : ngrok is checking token , <br>Note : if ngrok didn't get the token then paste that ngrok token from it's website
	- STEP 5.2 : press `n` - cuz we don't want to change ngrok-token
	- now php + ngrok server are getting started , output <br>![[21_lecture-4-M3.jpg | 500]]
- STEP 6 : copy & share the link with a Victim i.e "https: //3c46-223-190-13-157.ngrok-free.app" <br>- & say to Victim that open the link in his/her Kali
	...
	- if someone open the link & join - then output looks like : <br>![[21_lecture-5-M3.jpg | 500]]
...
- STEP 2 : how to use cloudflared tool with camphish tool
	...
	- STEP 2.2 : press `2` & then press `3` - then press `n` , output : <br>![[21_lecture-11-M3.jpg | 500]]
	...
	- STEP 2.3 : open a new terminal , run `sudo su` - then run `cloudflared --url http://localhost:3333` <br>- Note ✅ : here we added "http://" , not "https://" - cuz https needs SSL or TLS certificate to run ✔ <br>- output : <br>![[21_lecture-12-M3.jpg | 500]]
	...
	- output : we'll get Public Static IP addresses & their live cam pictures <br>![[21_lecture-13-M3.jpg | 300]]
